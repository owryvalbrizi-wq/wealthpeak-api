...
def deposit():
    ...
    desc = f"Pending deposit via {provider_name}"
    if phone:
        desc += f" ({phone})"
    desc += f" – ${amount}"
    ...
    return jsonify({
        "message": f"Deposit request recorded. Awaiting confirmation. Ref: {ref}",
...
def pawapay_initiate():
    ...
    if not is_configured():
        return jsonify({"error": "PawaPay is not configured on the server"}), 503
...
def pawapay_callback():
    ...
    completed = status in ("COMPLETED", "SUCCESS")  # ACCEPTED is not paid yet
